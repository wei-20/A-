...
class AStarPlanner(object):
    ...
    def collision(self, grid, grid_map):
        ...
        if grid[0] < radius_in_grid or grid[1] < radius_in_grid or grid[0] + radius_in_grid >= grid_map.max_x or grid[1] + radius_in_grid >= grid_map.max_y:
            return True
        xmin = int(grid[0] - radius_in_grid)
        xmax = int(grid[0] + radius_in_grid)
        ymin = int(grid[1] - radius_in_grid)
        ymax = int(grid[1] + radius_in_grid)

        for xi in range(int(xmin), int(xmax) + 1):
            for yi in range(int(ymin), int(ymax) + 1):
                if grid_map.grid_type(xi, yi) == 'occupied':
                    return True
    ...
    def get_path(self, start_grid, goal_grid, grid_map):
        ...
        while len(self.open_list) != 0:
            ...
            if abs(state_grid[0] - goal_grid[0]) < 0.5 and abs(state_grid[1] - goal_grid[1]) < 0.5 and int(abs((state_grid[2] - goal_grid[2])/0.206)) < 3:
                ...
                print('number of nodes:', self.valid_nodes)
                print('ReedsShepp_totalcost:',self.grids_info.get_g_cost(id_grid))
            ...
            for u in self.robot_model.get_controls(state_grid, grid_map):
                ...
                if succ not in self.open_list:
                    self.valid_nodes = self.valid_nodes + 1
    ...
    def show_path(self, topic_name, grid_map):
        ...
        for grid in self.path:
            ...
            msg.colors.append(ColorRGBA(r=0.8, a=1.0))
    ...
    def show_nodes(self, topic_name, grid_map):
        pub = rospy.Publisher(topic_name, Marker, queue_size=10, latch=True)
        ...
        for grid in self.path:
            ...
            y = (grid_map.max_y- grid[1]) * grid_map.resolution
            msg.points.append(Point(x=x, y=y))
            msg.colors.append(ColorRGBA(r=0.8, a=1.0))
